...
public class Test {
    ...
    public void test(){
        var adminId = createAdminAccount();
        System.out.println("Setting Allowance");
        allowance();
        System.out.println("Loading 100 users");
        var userIds = create100Users();
        System.out.println("Loading 100 channels");
        var channelIds = create100Channels(userIds, adminId);
        System.out.println("Approve 100 channels");
        approveChannels(channelIds);
        System.out.println("Loading 100 videos");
        var videoIds = create100Videos(adminId);
        System.out.println("Funding Channels");
        fundChannels(channelIds);
        System.out.println("Purchasing videos");
        purchaseContent(channelIds, videoIds, userIds);
        System.out.println("Daily Check Up");
        contentService.dailyCheckup();
        System.out.println("Changing Videos");
        for (String videoId: videoIds){
            var content = contentRepository.findById(videoId).orElseThrow();
            content.setSentEmails(true);
            content.setReleaseDate(LocalDate.now().minusDays(2));
            contentRepository.save(content);
        }
        System.out.println("Daily Check Up 2");
        contentService.dailyCheckup();
    }

    private void extendReleaseDate(){
        var Allcontent = contentRepository.findAll();
        for (Content content: Allcontent){
            content.setReleaseDate(LocalDate.now().plusDays(1));
            contentRepository.save(content);
        }
    }
    ...
    private void purchaseContent(List<String> channelIds, List<String> videoIds, ArrayList<String> userIds) {
        BigInteger gasPrice;
        BigInteger gasLimit = BigInteger.valueOf(5000000L);
        try {
            gasPrice = web3j.ethGasPrice().send().getGasPrice();
        ...
        for (String videoId: videoIds){
            ...
            if (isAuction(content.getContentType())){
                for (String userId: userIds){
                    ...
                    try {
                        transactionHash = bidServiceContract.placeBid(videoId, user.getUserId(), manaAmountInWei.toBigInteger()).send().getTransactionHash();
                ...
                if (swap){
                    for (int i = 0; i < 100; i++) {
                        payForContent(channelIds.get(i), videoId);
                        i++;
                    }
                    swap = false;
                }
                    ...
                    for (int i = 1; i < 100; i++) {
                        watchNowPayLater(channelIds.get(i), videoId);
                        i++;
                    }
                    swap = true;
                }
    ...
    private void watchNowPayLater(String channelId, String videoId) {
    ...
    private void payForContent(String channelId, String contentId){
        var channel = channelRepository.findById(channelId).orElseThrow();
        var highestAverageWeeklyViewers = getMaxNumber(channel.getStreamerInfo().stream().map(StreamerInfo::getAverageWeeklyViewers).collect(Collectors.toList()));
    ...
    private void payment(Users user, Content content, String manaAmount, int dollarAmount, String transactionHash){
        ...
        if(Payment != null){
            ...
            Payment.setDollarAmount((double) dollarAmount);
            ...
            var newPayment = Project_Noir.Athena.Model.Payment.builder()
                    ...
                    .dollarAmount((double) dollarAmount)
                    .refundDate(null)
        ...
        content.getListOfBuyerIds().put(user.getUserId(), manaAmount);
        content.setHype(content.getHype() + (dollarAmount * auctionModifier));
        contentRepository.save(content);
    }
    ...
    private ArrayList<String> create100Videos(String adminId) {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            Content.setYoutubeTrailerVideoID(("youtubeTrailerVideoID" + i));
    ...
    private void approveChannels(ArrayList<String> channelIds) {
        for (String channelId: channelIds){
            var channel = channelRepository.findById(channelId).orElseThrow();
    ...
    private ArrayList<String> create100Channels(ArrayList<String> userIds,String adminId){
    ...
    private ArrayList<String> create100Users(){
