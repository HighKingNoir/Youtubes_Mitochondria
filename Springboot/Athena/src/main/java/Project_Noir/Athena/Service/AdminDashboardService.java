...
public class AdminDashboardService {
    ...
    public List<AdminDashboardChartInfo> getYearChartData() {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            var allLogs = getContractLogsBetween(startDate, endDate);
    ...
    public ArrayList<Object> getTransactionCountChange() {
        var array = new ArrayList<>();
        Instant currentDate = Instant.now();
        Instant monthAgo = currentDate.minus(31, ChronoUnit.DAYS);
        Instant twoMonthsAgo = currentDate.minus(62, ChronoUnit.DAYS);

        var logsThisMonth = sivantisContractLogsRepository.findByCreationDateBetween(
                monthAgo,
                currentDate
        );

        var logsLastMonth = sivantisContractLogsRepository.findByCreationDateBetween(
                twoMonthsAgo,
                monthAgo
        );


        int totalTransactionCountThisMonth = logsThisMonth.stream().map(SivantisContractLogs::getTransactionCount).mapToInt(Integer::intValue).sum();
        int totalTransactionCountLastMonth = logsLastMonth.stream().map(SivantisContractLogs::getTransactionCount).mapToInt(Integer::intValue).sum();
        double percentageChange = 0.0; // Default to zero if totalManaAmountLastMonth is zero
        if (totalTransactionCountLastMonth != 0) {
            percentageChange = ((double) (totalTransactionCountThisMonth - totalTransactionCountLastMonth) / totalTransactionCountLastMonth) * 100;
        }
        array.add(totalTransactionCountThisMonth);
        array.add(percentageChange);
        return array;
    }


    private List<SivantisContractLogs> getContractLogsBetween(
            Instant startDate,
            Instant endDate){
        return sivantisContractLogsRepository.findByCreationDateBetween(
                startDate,
                endDate
        );
    }
