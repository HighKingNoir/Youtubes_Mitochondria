...
@Component({
  selector: 'app-most-hyped',
  templateUrl: './most-hyped.component.html',
  styleUrls: ['./most-hyped.component.css']
})
export class MostHypedComponent implements OnInit{
  private isScrollHandlerActive = false;
  Content: CreatedContentDetails[] = []
  private lastVideo = false
  private hypeValue?: number;
  private payLaterList: string[] = []
  private channel?: Channels
  private isLoggedIn: boolean;

  ngOnInit(): void {
    this.contentService.fetchAllActiveMostHypeContent(this.hypeValue).subscribe({
      next: (data:any) => {
        this.Content.push(...data);
        if (data.length == 50) {
          this.hypeValue = data[data.length - 1].hype;
          if(this.hypeValue && this.hypeValue <= .1){
            this.lastVideo = true
          }
