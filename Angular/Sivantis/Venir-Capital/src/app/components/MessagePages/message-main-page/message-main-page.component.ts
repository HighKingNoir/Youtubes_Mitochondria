...
export class MessageMainPageComponent implements OnInit{
  ...
  private processEnums(){
    ...
    else if(this.selectedEnum == MessageEnum.ChannelPayment || this.selectedEnum == MessageEnum.ChannelPendingPayment){
    ...
    else if(this.selectedEnum == MessageEnum.ChannelRefund || this.selectedEnum == MessageEnum.ChannelPendingRefund){
      this.manaAmount = Number(this.messageContent.extraInfo[3])
    }
    ...
    if(!this.messageContent.hasRead){
      ...
      this.authService.checkJWTExpiration(JWT).then((JWTResult) => {
        ...
        this.contentService.sentVideo(sentVideoRequestPayload).subscribe({
          next: () => {},
          error: (err) => {
            this.processingVideoEmails = false
  ...
  stripChannelPrefix(messageEnum: string): string {
    return messageEnum.replace(/^Channel/, '');
  }
    ...
    if (checkbox.checked) {
      ...
      try {
        ...
        if (this.clickedCheckboxIndices.length === this.messageContent.extraInfo.length - 4) {
          this.processingVideoEmails = true
          this.videoSent()
        }
      } catch (err) {
