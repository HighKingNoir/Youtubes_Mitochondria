...
@Component({
  selector: 'app-add-streamer-redirect',
  templateUrl: './add-streamer-redirect.component.html',
  styleUrl: './add-streamer-redirect.component.css',
  standalone: false
})
export class AddStreamerRedirectComponent {
  ...
  process(){
    ...
    const channelName = localStorage.getItem("addStreamerInfoRedirect")
    localStorage.removeItem("addStreamerInfoRedirect")
    const redirectURL = "/Channel/" + channelName
    switch (platform) {
      case 'Youtube':
      ...
      case 'Kick':
        const kickCode = this.route.snapshot.queryParamMap.get('code');
        const codeVerifier = localStorage.getItem('pkce_code_verifier');
        if(codeVerifier){
          if(kickCode){
              this.channelService.exchangeCodeForToken(kickCode, codeVerifier,  redirectURL.replace(/^\//, '')).subscribe({
                next: (response:any) => {
                  const access_token = response.access_token;
                  this.kickAPIService.getKickChannel(access_token, redirectURL)
                },
                error: () => {
                  this.alertService.addAlert("Token exchange failed", "danger")
                  this.router.navigateByUrl(redirectURL)
                }
              });
          }
