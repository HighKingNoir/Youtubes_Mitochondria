  ...
  swapTrailer(){
    if(this.googleAPIService.isLoggedIn()){
      ...
      this.modalService.open(SelectTrailerVideoComponent, { size: 'lg', scrollable: true, keyboard: false, animation: false, backdrop: 'static' }).result.then(() =>{
        const newTrailerId = this.videoInfo.getSelectedTrailerYoutubeVideoInfo().videoId
        if(newTrailerId == this.trailerID){
          return
        }
        this.trailerID = newTrailerId
        if(this.trailerID){
          this.youtubeAPI.fetchYoutTubeVideoByID(this.trailerID).subscribe((video: YoutubeVideoResponse) =>{
            if(video.items[0].status.privacyStatus === "private"){
              this.trailerMessage = "This trailer is set to private. Consider setting your trailer to 'public' to allow everyone to view it."
            }
            else{
